...
# Introduction
As exercise, I've decided to find a malware sample on MalwareBazaar to analyze. I was looking for a Windows malware written in C/C++, so I chose the sample described in this post. This is not a new threat on the web and after the analysis, with more knowledge about its functionalities, I can say that, probably, it's a variant of a Ghost RAT malware, known as Gh0stCringe. The sample is documented at [this](https://bazaar.abuse.ch/sample/04795669ada92df9a0361b6e169be7b6c23c888f6a56a96c35bd8cd4c09cd6c9/) link and here are some details:
...
![captured traffic](/assets/network_traffic.png)
...
![persistence through registry key](/assets/persistence.png)
...
![presistence with services](/assets/persistence_access_denied.png)
...
![malware import table](/assets/imports.png)
...
![DLL decrypted](/assets/dll_decrypted.png)
...
![DLL execution](/assets/dll_execution.png)
...
![taskkill rundll](/assets/kill_rundll.png)
...
![Thread to set persistence](/assets/persistence_asm.png)
...
![create_thread call](/assets/create_thread_call.png)
...
![create_thread assembly](/assets/create_thread_asm.png)

Finally, `thread_trampoline` takes the real function to run and executes it:

![Real function executed](/assets/thread_trampoline.png)
...
![Persistence all users](/assets/persistence_all_users.png)
...
![Keylog function calls](/assets/keylog_function_calls.png)
...
It's important to note the `call` at address `0x100066BA`, since, in the function called, a pointer is stored in a class previously created. This pointer is located at address `0x1000C30C` and it seems to be an array of function pointers. This will return during the C2 response analysis, where these functions are selected based on what the C2 server sends to the malware. 

![Class functions](/assets/class_functions.png)
...
![Drive loop](/assets/drive_loop.png)
...
![Inactive detection](/assets/inactive.png)
...
![Payload encryption](/assets/payload_encryption.png)
...
![Message decrypted](/assets/message_decrypted.png)
...
![Receive message and decrypt](/assets/recv_and_decrypt.png)
...
![Receive message and decrypt](/assets/final_call.png)

The content of the `edx` registry is the function pointer array mentioned early (addess `0x1000C30C`) and as you can see from the screenshot above, the second function of the array is called:

![Function array](/assets/function_array.png)
...
![Switch case](/assets/switch_case.png)
